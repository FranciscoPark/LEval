...
def _compute_default_rope_parameters(
    ...
    for i in range(len(base)):
        inv_freq = 1.0 / (base[i] ** (torch.arange(0, dim, 2, dtype=torch.int64).to(device=device, dtype=torch.float) / dim))
...
def _compute_linear_scaling_rope_parameters(
    ...
    inv_freq, attention_factor = _compute_default_rope_parameters(config, base, device, seq_len)
...
def _compute_llama3_parameters(
    config: PretrainedConfig, base,  device: "torch.device", seq_len: Optional[int] = None
) -> tuple["torch.Tensor", float]:
    """
